...
function solveFromHoles(holes: Point[], g: ReturnType<typeof emCouponGeometry>): EmAlignment {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    if (lu < 1 || lv < 1) {
      return fail('The detected coupon holes overlap; the scan is unreadable. Rescan at a higher resolution.')
    }
  ...
  // as its neighbours (derived from emCouponGeometry, not assumed).
